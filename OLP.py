...
def heldout_performance(path_to_data, path_to_results, n_depth, n_est):
    ...
    print("got here")
    print(auc_measure, precision_total[0], recall_total[0])
...
"""
Temporal Revision of the Original Algorithm Starts from Here.
"""

############################
def sample_true_false_edges_temporal(A_orig, A_true, A_train, A_ho,predict_num):  
    ...
    for i in range(predict_num-1, len(A_train)):
        ...
        if not os.path.isdir("./edge_tf_tr/"):
    	    os.mkdir("./edge_tf_tr/")
        np.savetxt("./edge_tf_tr/edge_t_{}.txt".format(i),temp_edge_t,fmt='%u')
        np.savetxt("./edge_tf_tr/edge_f_{}.txt".format(i),temp_edge_f,fmt='%u')
...
def gen_topol_feats_temporal(A_orig, A_tr, edge_s): 
    ...
    """
    time_count = []
    temp,time_cost = gen_topol_feats_timed(A_orig, A_tr[0],edge_s)
    time_count.append(time_cost)
    df_feat = temp
    for i in range(1,len(A_tr)):
        temp,time_cost = gen_topol_feats_timed(A_orig, A_tr[i],edge_s)
        time_count.append(time_cost)
        df_feat = pd.concat([df_feat, temp], axis=1)
    
    return df_feat, time_count

def topol_stacking_temporal(edges_orig, target_layer, predict_num): 
    ...
    #A_tr.append(A_train)
    #### extract features #####
    sample_true_false_edges_temporal(A, A_true, A_tr, A_hold, predict_num)
    ...
    for i in range(predict_num-1, len(A_tr)-1):
        ...
        df_temp, time_temp = gen_topol_feats_temporal(A, A_tr[i-predict_num+1 :i+1], edge_f_tr[i-predict_num+1])
        df_f_tr.append(df_temp)
        mytime.append(time_temp)
      
        df_temp, time_temp = gen_topol_feats_temporal(A, A_tr[i-predict_num+1 :i+1], edge_t_tr[i-predict_num+1])
...
def creat_full_set_temporal(df_t,df_f,predict_num):
    ...
    # data cleaning
    for i in range(1,predict_num):
        df_all.loc[df_all['short_path_{}'.format(i)] == np.inf,'short_path_{}'.format(i)] = 1000*max(df_all.loc[~(df_all['short_path'] == np.inf),'short_path'])
...
def gen_topol_feats_timed(A_orig, A, edge_s): 
    
    """ 
    The Only difference between this function and gen_topol_feats is that this one removed the PPR.
    To be cleaned later.
    """
