...
def calculate_ts_scores(edges_orig, target_layer, predict_num,name):
    ...
    edge_t_ho= np.loadtxt("./edge_tf_true/edge_t"+"_"+str(name)+".txt").astype('int')
    edge_f_ho= np.loadtxt("./edge_tf_true/edge_f"+"_"+str(name)+".txt").astype('int')
    df_f_ts, time1 = gen_topol_feats_temporal(A, A_tr, edge_f_ho)
    df_t_ts, time2 = gen_topol_feats_temporal(A, A_tr, edge_t_ho)

    false_ts = df_f_ts.values[:, :-44]
    test = np.reshape(false_ts, (false_ts.shape[0], para_u, 44)) 
    test = np.nan_to_num(test,  neginf=0,  posinf=0 , nan = 0)
    

    daterange = pd.date_range(start, periods=para_u)
    ts_false_matrix = []
    for j in range(false_ts.shape[0]):
        ...
        ts_false_matrix.append(temp)
        
    true_ts = df_t_ts.values[:, :-44]
    test_t = np.reshape(true_ts, (true_ts.shape[0], para_u, 44)) 
    test_t = np.nan_to_num(test_t,  neginf=0,  posinf=0 , nan = 0)

    ts_true_matrix = []
    for j in range(true_ts.shape[0]):
        ...
        for i in range(2,44):
            ...
            if not same:
                ...
                prediction = ts_feats[0]
            temp.append(prediction)
        ts_true_matrix.append(temp)
        
    X_timeseries = np.vstack((ts_false_matrix, ts_true_matrix))
    y_timeseries = [0]*false_ts.shape[0] + [1]*true_ts.shape[0]
    
    return X_timeseries, y_timeseries


def rf_with_chosen_feats(X,y, chosen_feat):
    clf = RandomForestClassifier(max_depth=2, random_state=0)
    if chosen_feat!=100:
        X_time = X[:,i]
    elif chosen_feat==100:
        X_time = X
    X_train_ts, X_test_ts, y_train_ts, y_test_ts = train_test_split(X_time, y, test_size=0.33, random_state=42)
    ...
    return auprc, auc_measure, precision_total[0], recall_total[0], cm_dt4
