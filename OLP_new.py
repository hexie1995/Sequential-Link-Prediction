...
def creat_numpy_files_final(dir_results, df_ho, df_tr, predict_num, sbm, lstm):
    ...
    full_feat_set = [None] * ((predict_num+1)*len(feature_set))
    len_of_feat = len(feature_set)
...
""
def top_final(edges_orig, target_layer, predict_num,name, lstm): 
    ...
    naive_y1 = A_tr[-1][edge_t_ho[:,0],edge_t_ho[:,1]].tolist()[0]
    naive_y2 = A_tr[-1][edge_f_ho[:,0],edge_f_ho[:,1]].tolist()[0]
    
    naive_y = naive_y1+naive_y2
    
    naive_true = np.ones(len(naive_y1)).tolist() + np.zeros(len(naive_y2)).tolist()
    
    naive_auc = roc_auc_score(naive_y, naive_true)
    naive_auprc = average_precision_score(naive_y, naive_true,'micro') 
    naive_precision, naive_recall, naive_f_measure_total, _ = precision_recall_fscore_support(naive_y, naive_true, average=None)
    ...
    df_tr = creat_full_set_final(df_t_tr_, df_f_tr_, predict_num)
    df_ho = creat_full_set_final(df_t_ho, df_f_ho, predict_num)
    ...
    out_order = ['TOP', 'TOP_withSBM', 'Top_withLSTM', 'TOP_withBOTH', 'SBM', 'LSTM', 'Naive']
    
    switch = [[0,0],[1,0],[0,1],[1,1]]
    ...
    for ss in range(4):
        ...
        a, b, c, d, e = auc_with_add_features(switch[ss][0], switch[ss][1], dir_output, df_ho, df_tr, predict_num, name)
        auprc.append(a)
        auc.append(b)
        precision.append(c)
        recall.append(d)
    ...
    #only compare with the sbm model
    only_sbm_auc  = roc_auc_score(df_ho.TP, df_ho["sbm"])
    sbm_auprc = average_precision_score(df_ho.TP, df_ho["sbm"],'micro') 
    #sbm_precision, sbm_recall, sbm_f_measure_total, _ = precision_recall_fscore_support(df_ho.TP, df_ho["sbm"], average=None)

    auprc.append(sbm_auprc)
    auc.append(only_sbm_auc)
    #precision.append(sbm_precision)
    #recall.append(sbm_recall)

    
    
    only_lstm_auc  = roc_auc_score(df_ho.TP, df_ho["lstm"])
    lstm_auprc = average_precision_score(df_ho.TP, df_ho["lstm"],'micro') 
    #lstm_precision, lstm_recall, lstm_f_measure_total, _ = precision_recall_fscore_support(df_ho.TP, df_ho["lstm"], average=None)

    auprc.append(lstm_auprc)
    auc.append(only_lstm_auc)
    #precision.append(lstm_precision)
    #recall.append(lstm_recall)
    
    
    auprc.append(naive_auc)
    auc.append(naive_auprc)
    precision.append(naive_precision)
    recall.append(naive_recall) 
    
    
    return out_order, auprc, auc, precision, recall , featim


def auc_with_add_features(sbm_switch,lstm_switch, dir_output, df_ho, df_tr, predict_num, name):
    
    creat_numpy_files_final( dir_output, df_ho, df_tr,predict_num, sbm_switch, lstm_switch)
    ...
    return auprc, auc, precision,recall, featim
